...
<body class="bg-gray-50 text-gray-800">
  ...
  <!-- Main Content -->
  <main id="mainAnim" class="  fade-in px-8 py-8 pb-32 max-w-7xl mx-auto">

    <!-- Canteen Section -->
    <div id="canteen" class="mb-12">
      <h2 class="text-2xl font-bold mb-6 text-gray-800">Canteen</h2>
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        @foreach($kantins as $kantin)
        <a href="{{ route('user.canteen', $kantin->id_kantin) }}" class="relative group cursor-pointer overflow-hidden rounded-2xl h-96 shadow-lg hover:shadow-xl transition">
          @if($kantin->gambar_kantin)
            <img src="{{ asset('storage/' . $kantin->gambar_kantin) }}" class="w-full h-full object-cover group-hover:scale-110 transition duration-300">
          @else
            <img src="https://images.unsplash.com/photo-1555939594-58d7cb561ad1?w=600" class="w-full h-full object-cover group-hover:scale-110 transition duration-300">
          @endif

          <div class="absolute bottom-0 left-0 right-0 p-6 text-white shadow-inner bg-gradient-to-t from-black/100 to-transparent rounded-b-2xl">
            <h3 class=" text-center text-2xl font-bold">{{ $kantin->nama_kantin }}</h3>
    ...
    <div>
      <h2 class="text-2xl font-bold mb-6 text-gray-800">Recommendations</h2>
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        @foreach($menus as $menu)
        <div class="menu-card bg-white rounded-2xl shadow-md hover:shadow-xl transition border-2 border-gray-200" data-menu-id="{{ $menu->menu_id }}">
          <div class="flex h-55">
            <!-- Left: Info -->
            <div class="flex-1 p-4 flex flex-col justify-between">
              <div>
                <h3 class="text-lg font-bold text-gray-800 mb-1">{{ $menu->nama_menu }}</h3>
                <p class="text-gray-500 text-sm line-clamp-2 mt-2">{{ $menu->deks_menu ?? 'Delicious food ready to enjoy!' }}</p>
              </div>
              <div>
                ...
                <!-- Default State: Harga besar & Add Order Button -->
                <div class="default-state">
                  ...
                  <button
                    ...
                    data-kantin="{{ $menu->kantin->nama_kantin ?? 'Unknown' }}">
                    Add Order
                  </button>
                </div>
            ...
            <div class="flex flex-col ml-2 mr-2 my-2">
              <div class="menu-image flex-shrink-0 transition-all duration-300 overflow-hidden rounded-lg" style="width: 134px; height: 100%;">
                @if($menu->gambar_menu)
              ...
              <button class="order-now hidden bg-yellow-400 hover:bg-yellow-500 text-gray-900 font-bold py-1.5 rounded-lg text-sm transition mt-1.5" style="width: 134px;">
                Order Now
              </button>
            </div>
          </div>
  ...
  <div id="successOverlay" style="display: none; background-color: rgba(0, 0, 0, 0); transition: background-color 0.3s ease; z-index: 99999;" class="fixed inset-0 flex items-end justify-center pb-64">
        ...
        <div>
          ...
          <div class="mr-350">
            ...
            <div style="width: 280px;">
              ...
              <!-- Warning pembayaran, absolute di bawah tombol, tidak menggeser tombol -->
              <div style="position: relative;">
                <div id="paymentWarning" style="display: none;" class="flex items-start gap-2 text-red-500 text-sm font-semibold mt-2 bg-transparent"></span>
  ...
  <script>
    // Jalankan animasi saat masuk dashboard
  ...
  buktiTransferInput.addEventListener('change', function(e) {
    ...
    confirmPaymentBtn.classList.add('bg-green-500', 'hover:bg-green-600', 'cursor-pointer');
  });
  ...
  function showQRView() {
    ...
    uploadStatus.classList.add('hidden');
    confirmPaymentBtn.disabled = true;
    confirmPaymentBtn.classList.remove('bg-green-500', 'hover:bg-green-600', 'cursor-pointer');
    confirmPaymentBtn.classList.add('bg-gray-400', 'cursor-not-allowed');
  }
  ...
  document.querySelectorAll('.addToCart').forEach(btn => {
    btn.addEventListener('click', function() {
      ...
      menuImage.style.height = '160px'; // Height diperpanjang
  ...
  document.querySelectorAll('.menu-card').forEach(card => {
    ...
    decreaseBtn.addEventListener('click', () => {
      ...
      if (cartItem) {
        cartItem.qty--;
        if (cartItem.qty <= 0) {
          // Remove from cart and reset card
  ...
  buyBtn.addEventListener('click', () => {
    ...
    // If Non-Tunai, show QR view (Step 3)
      ...
      selectedPaymentApp.textContent = selectedApp.toUpperCase();

      // Show QR View (Step 3 in cart bar)
      showQRView();
    }
  });
  ...
  function submitTransaction() {
    console.log('Sending payment data:', {
      ...
      items: cart,
      has_bukti: buktiTransferFile !== null
    });
